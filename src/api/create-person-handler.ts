...
import { CreatePersonUseCase } from "../application/create-person";
...
if (!tableName || !eventBusName) {
  throw new Error("Missing required environment variables: TABLE_NAME, EVENT_BUS_NAME");
}

const repository = new DynamoPersonRepository(dynamoClient, tableName);
const eventPublisher = new EventBridgeEventPublisher(eventBridgeClient, eventBusName);
const useCase = new CreatePersonUseCase(repository, eventPublisher);
...
export const handler = async (
  ...
  const logger = new RequestLogger(event.requestContext.requestId, "create-person");
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      logger.emit({ status: 400, error: { type: "SyntaxError", message: error.message } });
      return {
        statusCode: 400,
    ...
    if (error instanceof Error && error.name === "ZodError") {
      logger.emit({ status: 400, error: { type: "ZodError", message: error.message } });
      return {
        statusCode: 400,
